...
int main() {
    // Load the image 
    cv::Mat img1 = cv::imread("/Users/amtouti/Documents/IMG_1037.jpeg", cv::IMREAD_COLOR);
    cv::Mat img2 = cv::imread("/Users/amtouti/Documents/IMG_1038.jpeg", cv::IMREAD_COLOR);
    ...
    for(const auto& pt : disp_points_){
        size_of_pts++;
        centroid.x += pt.x;
        centroid.y += pt.y;
        centroid.z += pt.z;
    ...
    centroid /= size_of_pts;

    for(int i=0; i<disp_points_.size(); i++){
        if(cv::norm(centroid - disp_points_[i]) > 60.0){ 
            disp_points_.erase(std::next(disp_points_.begin(), i));
            colors.erase(std::next(colors.begin(), i));
            i--;
        }
    }
